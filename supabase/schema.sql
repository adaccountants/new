...
create policy cards_admin_all
  on public.cards
  for all
  using (auth.uid() in (select id from public.admins) or public.is_admin())
  with check (auth.uid() in (select id from public.admins) or public.is_admin());
...
create policy storage_admin_insert
  ...
  with check (
    ...
    and (auth.uid() in (select id from public.admins) or public.is_admin())
  );
